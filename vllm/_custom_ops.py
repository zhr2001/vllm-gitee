...
def cutlass_sparse_compress(a: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    a_meta = torch.empty(
        (m, k // 2 // elemsPerMetaElem), dtype=torch.uint8, device=a.device
    )
...
def moe_permute_before_all2all(
    topk_ids: torch.Tensor,
    num_experts: int,
    sorted_token_ids: torch.Tensor,
    ...
    torch.ops._moe_C.moe_permute_before_all2all(
        topk_ids, num_experts, sorted_token_ids, token_cnts, reversed_indices
    )


def moe_align_block_size_during_all2all(
    global_token_cnts: torch.Tensor,
    global_token_cnts_cumsum: torch.Tensor,
    num_experts: int,
    ep_size: int,
    block_size: int,
    token_block_expert_ids: torch.Tensor,
    expert_input_token_ids: torch.Tensor,
) -> None:
    torch.ops._moe_C.moe_align_block_size_during_all2all(
        global_token_cnts,
