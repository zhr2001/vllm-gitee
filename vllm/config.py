...
class ModelConfig:
    ...
    def _verify_cuda_graph(self) -> None:
        ...
        if self.hf_config.model_type == "deepseek_v3" and not self.enforce_eager:
            logger.warning(
...
class ParallelConfig:
    ...
    moe_tensor_model_parallel_size: int = 1
    ...
    def __post_init__(self) -> None:
        self.non_moe_world_size = (
            self.pipeline_parallel_size * self.tensor_parallel_size
        )
        self.moe_world_size = (
            self.moe_tensor_model_parallel_size
            * self.moe_expert_model_parallel_size
            * self.pipeline_parallel_size
        )
        self.world_size = (self.non_moe_world_size * self.moe_world_size) // math.gcd(
            self.non_moe_world_size, self.moe_world_size
        )
...
class CompilationConfig(BaseModel):
    ...
    def init_backend(self, vllm_config: "VllmConfig") -> Union[str, Callable]:
        ...
        if not self.cache_dir:
            ...
            if disabled:
                ...
                logger.info(
                    "Using cache directory: %s for vLLM's torch.compile", self.cache_dir
                )
    ...
    def init_with_cudagraph_sizes(self, sizes_to_specialize: List[int]):
        ...
        ignored_sizes = [
            x for x in self.candidate_compile_sizes if x not in self.capture_sizes
        ]
        if ignored_sizes:
            logger.warning(
                (
        ...
        self.max_capture_size = self.capture_sizes[0] if self.capture_sizes else 0

        # pre-compute the mapping from batch size to padded graph size
        self.bs_to_padded_graph_size = [0 for i in range(self.max_capture_size + 1)]
        for end, start in zip(self.capture_sizes, self.capture_sizes[1:] + [0]):
